...
install_demo() {
  ...
  kubectl apply -f ./src/zookeeperservice/deployment-staging.yaml -f $values -n "${namespace}"
  ...
  kubectl apply -f ./src/orderproducerservice/deployment-staging.yaml -f $values -n "${namespace}"
}
